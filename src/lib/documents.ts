...
export const uploadDocument = async ({ data }: { data: FormData }) => {
...
export const getDocumentUrl = async ({ data }: { data: { id: string } }) => {
    const store = loadStore();
...
export const deleteDocument = async ({ data }: { data: { id: string } }) => {
    const store = loadStore();
